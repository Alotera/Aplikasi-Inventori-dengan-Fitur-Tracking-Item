...
@section('title', __('warehouse.stock_history.title'))
@section('page-title', __('warehouse.stock_history.page_title'))
@section('page-description', __('warehouse.stock_history.page_desc'))
    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-800">
                ...
                {{ __('warehouse.stock_history.filter_heading') }}
            </h3>
        </div>
        <form method="GET" action="{{ route('warehouse-staff.stock-history') }}" class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label for="item_id" class="block text-sm font-medium text-gray-700 mb-2">{{ __('common.name') }}</label>
                    <select id="item_id" name="item_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        <option value="">{{ __('warehouse.stock_history.all_items') }}</option>
                ...
                <div>
                    <label for="movement_type" class="block text-sm font-medium text-gray-700 mb-2">{{ __('warehouse.stock_history.movement_type') }}</label>
                    <select id="movement_type" name="movement_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        <option value="">{{ __('warehouse.stock_history.all_types') }}</option>
                ...
                <div>
                    <label for="date_from" class="block text-sm font-medium text-gray-700 mb-2">{{ __('warehouse.stock_history.date_from') }}</label>
                ...
                <div>
                    <label for="date_to" class="block text-sm font-medium text-gray-700 mb-2">{{ __('warehouse.stock_history.date_to') }}</label>
    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-800">
                ...
                {{ __('warehouse.stock_history.table_heading') }}
            </h3>
        </div>
        ...
        <div class="overflow-x-auto">
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-500">{{ __('warehouse.stock_history.no_data') }}</p>
                </div>
            @endif
        </div>
    </div>
