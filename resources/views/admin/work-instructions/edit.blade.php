...
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <form method="POST" action="{{ route('admin.work-instructions.update', $workInstruction) }}" id="wi-form">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" required
                                    ...
                                    <option value="checking" {{ old('type', $workInstruction->type) === 'checking' ? 'selected' : '' }}>Checking</option>
                                    <option value="ambil" {{ old('type', $workInstruction->type) === 'ambil' ? 'selected' : '' }}>Ambil</option>
                                </select>
                                @error('type')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <textarea name="dropoff_notes" id="dropoff_notes" rows="3"
                                              ...
                                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('dropoff_notes') border-red-300 @enderror">{{ old('dropoff_notes', $workInstruction->dropoff_notes) }}</textarea>
                        ...
                        <div class="mt-6">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $workInstruction->title) }}" required
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('description') border-red-300 @enderror">{{ old('description', $workInstruction->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div>
                                ...
                                <select name="assigned_user_id" id="assigned_user_id" required
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ old('assigned_user_id', $workInstruction->assigned_user_id) == $user->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="deadline" id="deadline" value="{{ old('deadline', $workInstruction->deadline->format('Y-m-d\TH:i')) }}" required
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const existingItems = @json($workInstruction->items->mapWithKeys(function($item) {
        return [$item->id => $item->pivot->required_quantity];
    }));
    let itemCount = 0;
        ...
        itemRow.innerHTML = `
            <div class="flex-1">
                <select name="items[${itemCount}][item_id]" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    ...
                    ${items.map(item => `<option value="${item.id}" ${item.id == itemId ? 'selected' : ''}>${item.item_code} - ${item.name}</option>`).join('')}
                </select>
            </div>
            <div class="w-32">
                <input type="number" name="items[${itemCount}][required_quantity]" placeholder="Qty" min="1" value="${quantity}" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
            </div>
            <div>
    ...
    Object.keys(existingItems).forEach(itemId => {
        addItemRow(itemId, existingItems[itemId]);
    });
